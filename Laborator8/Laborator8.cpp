...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	float myAngle = glGetUniformLocation(shader->program, "angle");
	glUniform1f(myAngle, angle);

	GLint isSpotlight = glGetUniformLocation(shader->program, "isSpotlight");
	glUniform1i(isSpotlight, isSpot);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		auto tmp = glm::rotate(glm::mat4(1), x, up);
		tmp = glm::rotate(tmp, y, right);
		lightDirection = glm::vec3(tmp * glm::vec4(glm::vec3(0, -1, 0), 0));
	}
}
