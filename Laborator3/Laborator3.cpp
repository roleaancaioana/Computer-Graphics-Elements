...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	glm::ivec2 resolution = window->GetResolution();
	{
		...
		if (goToTheRight)
		{
			translateX += deltaTimeSeconds * SPEED * resolution.x;
			modelMatrix *= Transform2D::Translate(translateX, translateY);

			if (translateX > resolution.x * 0.3f)
			{
				goToTheRight = false;
			}
	...
	
	{
		...
		modelMatrix *= Transform2D::Translate(100.f / 2.f, 100.f / 2.f);
	...
	//TODO create animations by multiplying current transform matrix with matrices from Transform 2D
	{
		...
		modelMatrix *= Transform2D::Translate(100.f/2, 100.f/2);

		if (auxScale == true)
		{
			counter++;
			scaleX += deltaTimeSeconds * 1.5f;
			scaleY += deltaTimeSeconds * 1.5f;

			if (counter == LIMIT_SCALE)
			{
				auxScale = false;
			}
		} else 
