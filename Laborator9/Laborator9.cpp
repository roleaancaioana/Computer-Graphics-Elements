...
void Laborator9::Update(float deltaTimeSeconds)
{
...

	{
		...
		should_mix = 1;
		RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix, mapTextures["grass"], mapTextures["random"]);
		should_mix = 0;
	}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	int locationMix = glGetUniformLocation(shader->program, "should_mix");
	glUniform1i(locationMix, should_mix);

	int locationClock = glGetUniformLocation(shader->program, "Clock");

	if (mesh == meshes["sphere"]) {
		glUniform1f(locationClock, Engine::GetElapsedTime());
	}
	else {
		glUniform1f(locationClock, 0);
	}
